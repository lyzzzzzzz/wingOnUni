...
(function(win, com, $) {
	...
	function lazyLoad(doc, cb) {
		...
		com.myasync( /*makeArray(imgs)*/ $.slice.call(imgs), function(img, next) {
			...
			var data_src = img.getAttribute('src');
			var file = myStorage.getItem("filePathCache_" + common.hashCode(data_src));
			plus.io.resolveLocalFileSystemURL(file, function() {}, function() {
				img.setAttribute('src', '../../img/default.png');
			});
			if (data_src == null || (data_src && (data_src.indexOf('http://') < 0 && data_src.indexOf('https://') < 0 &&
					data_src.indexOf('file://') < 0))) {
				//console.log(data_src)
				img.setAttribute('src', '../../img/default.png');
			}

			if (data_src && (data_src.indexOf('http://') >= 0 || data_src.indexOf('https://') >= 0)) {
